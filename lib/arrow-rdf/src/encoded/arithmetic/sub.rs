...
impl EncScalarBinaryUdf for EncSub {
    ...
    fn evaluate(
        ...
        match XsdNumericPair::with_casts_from(lhs, rhs) {
            XsdNumericPair::Int(lhs, rhs) => match lhs.checked_sub(rhs) {
                None => collector.append_null()?,
                Some(value) => collector.append_int(value)?,
            },
            XsdNumericPair::Integer(lhs, rhs) => match lhs.checked_sub(rhs) {
                None => collector.append_null()?,
            ...
            XsdNumericPair::Decimal(lhs, rhs) => match lhs.checked_sub(rhs) {
                None => collector.append_null()?,
...
impl ScalarUDFImpl for EncSub {
    ...
    fn invoke_batch(
        ...
        dispatch_binary::<EncSub>(args, number_rows)
    }
}
