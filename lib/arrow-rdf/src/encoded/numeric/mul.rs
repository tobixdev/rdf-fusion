...
impl EncScalarBinaryUdf for EncMul {
    ...
    fn evaluate(
        ...
        match XsdNumericPair::with_casts_from(lhs, rhs) {
            XsdNumericPair::Int(lhs, rhs) => match lhs.checked_mul(rhs) {
                None => collector.append_null()?,
                Some(value) => collector.append_int(value)?,
            },
            XsdNumericPair::Integer(lhs, rhs) => match lhs.checked_mul(rhs) {
                None => collector.append_null()?,
            ...
            XsdNumericPair::Decimal(lhs, rhs) => match lhs.checked_mul(rhs) {
                None => collector.append_null()?,
...
impl ScalarUDFImpl for EncMul {
    ...
    fn invoke_batch(
        ...
        dispatch_binary::<EncMul>(args, number_rows)
    }
}
