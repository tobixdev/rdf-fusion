...
impl EncScalarUnaryUdf for EncAbs {
    type Arg<'data> = XsdNumeric;
    type Collector = EncRdfTermBuilder;

    fn evaluate(&self, collector: &mut Self::Collector, value: Self::Arg<'_>) -> DFResult<()> {
        match value {
            XsdNumeric::Int(value) => match value.checked_abs() {
                Some(value) => collector.append_int(value)?,
                None => collector.append_null()?,
            },
            XsdNumeric::Integer(value) => match value.checked_abs() {
