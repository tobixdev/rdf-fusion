    ...
    pub fn new() -> Self {
        Self {
            signature: Signature::new(
                TypeSignature::Exact(vec![RdfValueEncoding::data_type()]),
...
impl ScalarUDFImpl for EncAsNativeBoolean {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs<'_>) -> DFResult<ColumnarValue> {
        ...
        let boolean_array = as_boolean_array(terms.child(RdfValueEncodingField::Boolean.type_id()));
        let null_array = terms.child(RdfValueEncodingField::Null.type_id());
        ...
        let result = terms
            ...
            .enumerate()
            .map(|(idx, tid)| {
                Some(if *tid == RdfValueEncodingField::Boolean.type_id() {
