...
pub fn encode_string(value: String) -> ScalarValue {
    ...
    ScalarValue::Union(
        Some((RdfValueEncodingField::String.type_id(), Box::new(value))),
...
pub fn encode_scalar_null() -> ScalarValue {
    ScalarValue::Union(
        Some((
            RdfValueEncodingField::Null.type_id(),
...
pub fn encode_scalar_named_node(node: NamedNodeRef<'_>) -> ScalarValue {
    ...
    ScalarValue::Union(
        Some((RdfValueEncodingField::NamedNode.type_id(), Box::new(value))),
        RdfValueEncoding::fields(),
        UnionMode::Dense,
    )
}
...
pub fn encode_scalar_blank_node(node: BlankNodeRef<'_>) -> ScalarValue {
    ...
    ScalarValue::Union(
        Some((RdfValueEncodingField::BlankNode.type_id(), Box::new(value))),
        RdfValueEncoding::fields(),
        UnionMode::Dense,
    )
}
...
fn specialize_string(literal: LiteralRef<'_>, value: &str) -> ScalarValue {
    ...
    let structs = StructArray::new(RdfValueEncoding::string_fields(), arrays, None);
    let scalar_value = ScalarValue::Struct(Arc::new(structs));

    ScalarValue::Union(
        Some((
            RdfValueEncodingField::String.type_id(),
            Box::new(scalar_value),
        )),
        RdfValueEncoding::fields(),
        UnionMode::Dense,
    )
...
fn handle_generic_literal(literal: LiteralRef<'_>) -> ScalarValue {
    ...
    let structs = StructArray::new(RdfValueEncoding::typed_literal_fields(), arrays, None);
    let scalar_value = ScalarValue::Struct(Arc::new(structs));
    ScalarValue::Union(
        Some((
            RdfValueEncodingField::OtherTypedLiteral.type_id(),
            Box::new(scalar_value),
        )),
        RdfValueEncoding::fields().clone(),
        UnionMode::Dense,
    )
}
