...
impl ScalarBinaryRdfOp for SubRdfOp {
    ...
    fn evaluate<'data>(
        ...
        match NumericPair::with_casts_from(lhs, rhs) {
            NumericPair::Int(lhs, rhs) => lhs.checked_sub(rhs).map(Numeric::Int).ok_or(()),
            NumericPair::Integer(lhs, rhs) => lhs.checked_sub(rhs).map(Numeric::Integer).ok_or(()),
            NumericPair::Float(lhs, rhs) => Ok(Numeric::Float(lhs - rhs)),
            NumericPair::Double(lhs, rhs) => Ok(Numeric::Double(lhs - rhs)),
            NumericPair::Decimal(lhs, rhs) => lhs.checked_sub(rhs).map(Numeric::Decimal).ok_or(()),
        }
    }
}
